...
public class TitleItemDecoration extends RecyclerView.ItemDecoration {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (position > -1) {
                if (position == 0) {//等于0的时候绘制title
                    ...
                    if (null != PinyinUtils.getFirstCharacter(mData.get(position).getName()) &&
                            !PinyinUtils.getFirstCharacter(mData.get(position).getName()).equals(PinyinUtils.getFirstCharacter(mData.get(position - 1).getName()))) {
    ...
    private void drawTitle(Canvas c, int left, int right, View child, RecyclerView.LayoutParams params, int position) {
        ...
        mPaint.getTextBounds(PinyinUtils.getFirstCharacter(mData.get(position).getName()), 0, PinyinUtils.getFirstCharacter(mData.get(position).getName()).length(), mBounds);
        c.drawText(PinyinUtils.getFirstCharacter(mData.get(position).getName()),
    ...
    public void onDrawOver(Canvas c, final RecyclerView parent, RecyclerView.State state) {
        ...
        String tag = PinyinUtils.getFirstCharacter(mData.get(position).getName());
        View child = parent.findViewHolderForLayoutPosition(position).itemView;
        //Canvas是否位移过的标志
        boolean flag = false;
        if ((position + 1) < mData.size()) {
            //当前第一个可见的Item的字母索引，不等于其后一个item的字母索引，说明悬浮的View要切换了
            if (null != tag && !tag.equals(PinyinUtils.getFirstCharacter(mData.get(position + 1).getName()))) {
